...
class SIM_CORE(Node):
    ...
    def publish_data(self):
        ...
        while i < len(self.sensor_events) - 1:
            ...
            # self.get_logger().info(f"Time: {timestamp}, Next: {next_timestamp}")

            msg = Header()
            msg.stamp.sec = int(timestamp)
            msg.stamp.nanosec = int((timestamp - int(timestamp))*1e+09)
            ...
            else: # In case there are multiple data at the same time 
                while sleep_time == 0:
                    i+=1
                    timestamp, sensor_id = self.sensor_events[i]
                    next_timestamp = self.sensor_events[i + 1][0]
